...
if (isset($_POST['licence']) && $_POST['licence'] != "" && 
    ...
    $people_result = mysqli_query($conn, $people_id);
    $vehicle_result = mysqli_query($conn, $vehicle_id);

    if (mysqli_num_rows($people_result) > 0 && mysqli_num_rows($vehicle_result) > 0) {
        $people_data = mysqli_fetch_assoc($people_result);
        $vehicle_data = mysqli_fetch_assoc($vehicle_result);

        $People_licence = $people_data['People_licence'];
        $Vehicle_plate = $vehicle_data['Vehicle_plate'];
     
        $sql = "INSERT INTO Ownership (People_ID, Vehicle_ID) VALUES (" . $people_data["People_ID"] . ", " . $vehicle_data["Vehicle_ID"] . ");";

        if (mysqli_query($conn, $sql)) {
            $detail = "Insert record: Plate=$Vehicle_plate, Licence=$People_licence";
            $history_sql = "INSERT INTO history (username, type, `database`, action, detail, timestamp) 
                            VALUES ('" . $_SESSION['name'] . "', '" . $_SESSION['username_type'] . "', 'Ownership', 'Insert', '$detail', NOW());";
            mysqli_query($conn, $history_sql);
          
            header("Location:newownership.php");
            exit();
        } else {
            $error[] = 'Failed to insert ownership record.';
        }
    } else {
        if (mysqli_num_rows($people_result) > 0 && mysqli_num_rows($vehicle_result) == 0) {
            $error[] = 'Incorrect plate! Please check if the information exists or is correct!';
        } elseif (mysqli_num_rows($people_result) == 0 && mysqli_num_rows($vehicle_result) > 0) {
            $error[] = 'Incorrect licence! Please check if the information exists or is correct!';
        } else {
            $error[] = 'Incorrect licence and plate! Please check if the information exists or is correct!';
        }
    }
}
...
<h1 style="font-family:fantasy;text-align:center; font-size: 1.5cm;" class="mt-3">New Ownership</h1>
